...
export async function getServerSideProps(ctx: GetServerSidePropsContext) {
  ...
  const providerId = ctx.query.provider;
  ...
  const provider: APIResponse<Provider> = await fetch(
    `${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/auth/providers/${providerId}`,
    {
      headers: {
      ...
      {provider ? (
        <>
          ...
          <h3 className="text-base font-medium text-center text-white">
            ...
            {provider.provider_data.username ?? provider.provider_id}
          </h3>

          <form
            action={`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/auth/logout/${provider.type}`}
            method="POST"
          >
            <button
          ...
          <Link href="/" passHref>
            <a>
              <p className="w-full my-4 text-sm font-bold text-center text-blue-500">
                Go Back
              </p>
            </a>
          </Link>
