...
export function GetDiscordProfilePicture(id: string): string {
  return `https://cdn.discordapp.com/avatars/${id}/${id}.png`;
}
...
export function calculateMilestone(number: number) {
  // use math to calculate the next milestone
  const base = Math.pow(10, Math.floor(Math.log(number) / Math.log(10)));
  const milestone = Math.ceil(number / base) * base;
  const gap = milestone - number;
  return {
    milestone,
    gap,
  };
